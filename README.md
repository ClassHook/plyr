...
| `markers`           | Object                     | `{ enabled: false, points: [{ time, tip?, tipHtml?, onClick? }] }`                                                                           | `enabled`: Toggles if markers should be shown by default. `points`: An array of objects where the markers will appear. `onClick`: A custom event handler for when the marker is clicked. The player and the point is passed in as arguments. By default, seeks the player to the marker time. Each object should have a `time` and either a `tip` or `tipHTML` key. For example: `[{ time: 10, tip: 'Test tip' }]`. `time` refers to the seconds value where the marker should appear. `tip` is a plaintext tooltip that appears when the user hovers over the marker. Use `tipHTML` if you want to display an HTML tooltip. If both `tip` and `tipHTML` are empty, the marker will be shown, but tooltips will not appear.                                                                                     |
...
| `youtube`            | Object                     | `{ noCookie: false, eduPlayer: false, rel: 0, showinfo: 0, iv_load_policy: 3, modestbranding: 1 }`                                               | See [YouTube embed options](https://developers.google.com/youtube/player_parameters#Parameters). The only custom options are `noCookie` to use an alternative to YouTube that doesn't use cookies (useful for GDPR, etc) and `eduPlayer` to use the YouTube Player for Education. Some are set automatically based on other config options, namely: `autoplay`, `hl`, `controls`, `disablekb`, `playsinline`, `cc_load_policy`, `cc_lang_pref`, `widget_referrer` |
...
| `markers`           | ✓      | ✓      | Gets or sets the markers for the player. See [options](#options) above for examples.|
