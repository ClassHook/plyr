...
| `markers`           | Object                     | `{ enabled: false, onClick?, points: [{ time, tip?, tipHtml? }] }`                                                                           | `enabled`: Toggles if markers should be shown by default. `points`: An array of objects where the markers will appear. `onClick`: A custom event handler for when a marker is clicked. The player and the point is passed in as arguments. By default, seeks the player to the marker time. Each object should have a `time` and either a `tip` or `tipHTML` key. For example: `[{ time: 10, tip: 'Test tip' }]`. `time` refers to the seconds value where the marker should appear. `tip` is a plaintext tooltip that appears when the user hovers over the marker. Use `tipHTML` if you want to display an HTML tooltip. If both `tip` and `tipHTML` are empty, the marker will be shown, but tooltips will not appear.                                                                                     |
