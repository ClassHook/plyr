typeof navigator === "object" && (function (global, factory) {
  ...
  const dailymotion = {
    setup() {
      ...
      if (is.object(window.DM) && is.function(window.DM.player)) {
        ...
        loadScript(player.config.urls.dailymotion.sdk).then(() => {
          dailymotion.ready.call(player);
        }).catch(error => {
    ...
    ready() {
      ...
      const player = this; // Getting dailymotion config

      const config = player.config.dailymotion; // Get the source URL or ID if it's an <iframe />
      ...
      const videoId = parseId$2(source); // Get poster, if already set

      const {
        poster
      } = player; // This div will be replaced by an iframe by DM.player

      const container = createElement('div'); // We need to keep the previous element to implement the destroy method and keep the same behaviour as Youtube and Vimeo

      const previousElement = player.media.cloneNode();
      player.media = replaceElement(container, player.media); // Get poster image

      fetch(format(player.config.urls.dailymotion.api, videoId), 'json').then(response => {
      ...
      player.embed.addEventListener('apiready', () => {
        ...
        player.embed.addEventListener('ended', () => {
          assurePlaybackState$2.call(player, false); // DailyMotion doesn't support loop for a single video, so mimick it.
  ...
  class Plyr {
    ...
    get buffered() {
      const {
        buffered
      } = this.media; // YouTube / Vimeo / DailyMotion return a float between 0-1
